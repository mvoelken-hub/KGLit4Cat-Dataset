...
| --- | --- | --- |
| `IR-IR.zip` | Completed through the automatic endpoint. | The generated report was schema-valid with top-1 file ranking hit, top-3 file recall `0.75`, source trace coverage `1.0`, profile field coverage `1.0`, object macro F1 `0.2333`, attribute F1 `0.1333`, and vocabulary mapping F1 `0.0`. |
...
The IR result was produced before the deterministic resource-inventory fix, so it should be rerun before treating the metrics as final. It is still useful because it shows the complete endpoint can produce a schema-valid profile artifact and enough persisted state for scoring.
...
- Schema-valid profile projection is reachable on at least one real package.
...
- Rerun `IR-IR.zip` after the resource-inventory fix and compare metrics.
