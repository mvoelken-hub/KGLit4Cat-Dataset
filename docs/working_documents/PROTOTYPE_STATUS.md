...
| Semantic reconstruction | After semantic requirement assessment, projection asks the LLM for minimal schema-constrained write envelopes over provenance, technical agents, method plan, instrument settings, dataset identity, and aboutness. The output schema is sliced to only the allowed target paths and reachable `$defs`, then deterministic code creates missing containers, applies upsert writes, validates each edit, salvages valid writes, and records `semantic_reconstructions` in the requirement report. |
