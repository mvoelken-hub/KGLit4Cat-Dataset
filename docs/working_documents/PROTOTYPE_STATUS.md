...
| Semantic reconstruction | After semantic requirement assessment, projection runs a semantics-driven draft-field iteration. Backend cleanup first compiles same-parent duplicate-attribute merges and bad range decomposition. LLM calls then diagnose defects with a small `defects[]` schema, and only separate synthesis calls may create small target values. Deterministic code compiles diagnosis into append/replace/remove/merge actions, creates containers, validates each accepted action, salvages valid actions, and records diagnosis/action/synthesis counts plus rejected reasons in `semantic_reconstructions`. Defective requirements with no safe action are recorded as `unresolved`, not skipped. |
