...
| Semantic reconstruction | After semantic requirement assessment, projection asks the LLM for minimal schema-constrained action envelopes over provenance, technical agents, method plan, attribute parent suitability, instrument settings, dataset identity, and aboutness. The output schema is sliced to only the allowed target paths and reachable `$defs`; aboutness uses a lean `id`/`title`/`description` slice. Deterministic code creates missing containers, applies append/replace/remove/merge actions, rejects same-parent duplicate semantic slots, validates each accepted action, salvages valid actions, and records action counts plus rejected reasons in `semantic_reconstructions`. |
