...
| Semantic reconstruction | After semantic requirement assessment, projection asks the LLM for minimal JSON Patch reconstruction operations over provenance, technical agents, method plan, instrument settings, dataset identity, and aboutness. It uses only the current draft and existing requirement artifacts, rejects disallowed paths, validates each edit, rolls back invalid edits, and records `semantic_reconstructions` in the requirement report. |
