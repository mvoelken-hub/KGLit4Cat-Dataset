...
| Semantic reconstruction | After semantic requirement assessment, projection runs a semantics-driven draft-field iteration. Backend cleanup compiles same-parent duplicate merges, description/sibling-grounded range decomposition, verified cross-parent duplicate removal, and evaluated-activity self-reference removal. Dataset aboutness and per-activity evaluation targets are projected and judged as different claims; runtime target paths cover every generation activity, either evaluated relation family fulfills the target requirement, missing targets are never synthesized, unsupported mirrored edges can be removed independently, and shared IDs remain valid only for unambiguous co-reference. Measurement-signal and measurement-condition evidence routes to evaluated-entity attribute paths when an evaluated entity exists, while instrument and setting evidence routes to the activity or agent path; the semantic router sees evaluated_entity context in the draft excerpt and chooses among expanded allowed target paths. Rounded numeric variants merge only when canonical quantity and compatible units agree. LLM diagnosis and synthesis use separate contracts; synthesis schemas come from the exact profile target path and absence placeholders are rejected. Collection-wide attribute replacement and synthesis-backed mechanical actions are rejected. Forced rebuilds clear stale curated/report state, and final report guards run against the delivered grounded document. Requirement-specific evidence limits keep mechanical checks draft-only and placement prompts small. Deterministic code validates each action, salvages valid actions, prevents evidence-only false fulfillment, and persists diagnosed defects, compiled actions, applied/rejected counts, and reasons. Defective requirements with no safe action are recorded as `unresolved`, not skipped. |
