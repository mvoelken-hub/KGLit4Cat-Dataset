...
| SIMONE grounds selected metadata elements against semantic vocabularies. | Partly supported | Neo4j vocabulary import/search, vector/full-text retrieval, candidate selection via an LLM query-formulation + selection step using document context. Domain-agnostic by design; small-model difficult cases abstain (raw value kept) instead of fabricating. See Difficult Grounding Cases below. | Say "selected fields/attributes"; avoid claiming comprehensive ontology coverage; acknowledge difficult cases are left raw rather than mis-grounded. |
