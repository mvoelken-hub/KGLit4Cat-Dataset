...
- Use only these categories: resource_signal, method_signal, measurement_signal, agent_signal, activity_signal, instrument_signal, surrounding_signal, other.
...
- Use measurement_signal for primary/raw data values, axis bounds, point counts, min/max values, or row-like observations; these are evidence-only and should not become metadata attributes.
- Use agent_signal only for software, devices, instruments, machines, services, or executable systems that can carry out work.
...
- Numeric values are measurement_signal when they are observed data values, axis bounds, point counts, extrema, or row-like observations; use instrument_signal only when the number configures a setting, parameter, threshold, unit, or processing choice.
