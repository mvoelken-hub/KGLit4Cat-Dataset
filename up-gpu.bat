...
echo   2. Development GPU - Neo4j and Ollama run in Docker with GPU acceleration, API and frontend run locally
...
docker compose --env-file .env.development -f docker-compose.dev.yml -f docker-compose.gpu.yml stop frontend >nul 2>nul
docker compose --env-file .env.development -f docker-compose.dev.yml -f docker-compose.gpu.yml up -d --build neo4j ollama
