...
5. `TaskRegistry` - 152 edges
6. `BaseModel` - 133 edges
7. `TaskType` - 130 edges
8. `DataSourceService` - 126 edges
9. `RunUsage` - 121 edges
10. `SemanticService` - 113 edges
...
Cohesion: 0.07
Nodes (26): Any, CurationLedgerRecord, DraftValidationResult, EvidenceContext, ExtractionFileSummary, ExtractionOverview, ExtractionOverviewStatus, ExtractionRunResult (+18 more)
...
Cohesion: 0.14
Nodes (23): DataSourceService, UploadFile, DataPackage, ChunkingRequest, description, Query, ChunkingRequest, ChunkRequestResponse (+15 more)
...
Cohesion: 0.13
Nodes (8): Unit tests for the tunable text-quality classifier., Without a config the classifier must behave exactly as before for non-structured, The regex fix means ##.DELAY= … now gets structured_text bonus by default., Raising symbol_ratio_threshold to 0.50 keeps ##.DELAY= (6.5, 6.5)., Raising keep_score_threshold can still drop a borderline structured line., Lines starting with ##. or ##$ and containing a key=value should get the structu, A line of many numbers should still be dropped even with a lenient config., TextQualityConfigTests
...
- **Are the 198 inferred relationships involving `TaskStatus` (e.g. with `InitialTasks` and `Any`) actually correct?**
  _`TaskStatus` has 198 INFERRED edges - model-reasoned connections that need verification._
- **Are the 141 inferred relationships involving `OllamaClientWrapper` (e.g. with `InitialTasks` and `Any`) actually correct?**
  _`OllamaClientWrapper` has 141 INFERRED edges - model-reasoned connections that need verification._
- **Are the 145 inferred relationships involving `Settings` (e.g. with `InitialTasks` and `AsyncClient`) actually correct?**
  _`Settings` has 145 INFERRED edges - model-reasoned connections that need verification._